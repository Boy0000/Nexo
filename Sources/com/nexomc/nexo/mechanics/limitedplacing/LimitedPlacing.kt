...
class LimitedPlacing(section: ConfigurationSection) {
    ...
    fun isNotPlacableOn(block: Block, blockFace: BlockFace): Boolean {
        val placedBlock = block.getRelative(blockFace)
    ...
    private fun checkIfNexoItem(block: Block) = (NexoBlocks.customBlockMechanic(block.blockData)
        ?: NexoFurniture.furnitureMechanic(block.location))?.itemID
